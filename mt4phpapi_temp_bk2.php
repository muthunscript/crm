 ...
 class mt4api1
 {
 	...
 	public function open_account($group, $name, $passsword, $email, $agent, $country, $state, $city, $address, $phone, $phone_password, $status, $zipcode, $id, $leverage, $comment, $investor, $login = '')
 	{
		...
		else if(!in_array($site,$tr_array)&&$this->mode == 1)
		{
 		...
 		else if (isset($_site_config['exe']) && isset($_exe_array[$_site_config['exe']]))
 		{
			...
			open_account:
 		...
 		else if (isset($account_socket) && $account_socket['status'])
 			{
				$log->info("19");
 				if ($this->mode == 1)
 				{
 			...
 			else
 			{
				...
				$log->info("else......open_account_old");
	...
	public function deposit_test($login, $deposit, $comment)
 	...
 	public function withdraw_old($login, $withdraw)
 	{
 		//echo $this->mode;
	...
	public function withdraw($login, $withdraw)
		...
		$comment='ok';
 		if (isset($deposit_socket) && $deposit_socket['status'])
 		{
 			...
 			if ($this->mode == 1)
 			{
 				...
 				$q = DR . DS . 'dep' . DS . 'dep.exe "' . $deposit_socket["real"] . ':443" "' . $deposit_socket["ra"] . '" "' . $deposit_socket["rp"] . '" "' . $login . '" "' . -$withdraw . '" "' . $comment . '"';
 				//loge(encrypt($q));
				$n='';
 				$n = exec_exe1(DR . DS . 'dep' . DS . 'dep.exe "' . $deposit_socket["real"] . ':443" "' . $deposit_socket["ra"] . '" "' . $deposit_socket["rp"] . '" "' . $login . '" "' . -$withdraw . '" "' . substr($comment, -30) . '"');
				$a=$n[1];
 				//loge($a);
 				//loge(encrypt(json_encode($n)));
 			...
 			else
 			{
 				if (!in_array($_site_config['sitename'], $tr_array))
 				{
 					...
 					$q = DR . DS . 'dep' . DS . 'dep.exe "' . $deposit_socket["demo"] . ':443" "' . $deposit_socket["da"] . '" "' . $deposit_socket["dp"] . '" "' . $login . '" "' . -$withdraw . '" "' . $comment . '"';
 					//loge(encrypt($q));
					$n='';
 					$n = exec_exe1(DR . DS . 'dep' . DS . 'dep.exe "' . $deposit_socket["demo"] . ':443" "' . $deposit_socket["da"] . '" "' . $deposit_socket["dp"] . '" "' . $login . '" "' . -$withdraw . '" "' . substr($comment, -30) . '"');
					$a=$n[1];
 					//loge($a);
 					//loge(encrypt(json_encode($n)));
 	...
 	public function change_leverage($login, $levarage)
 	{
		...
		elseif(!in_array($site,$tr_array)&&!in_array($site,$updemo_array)&&$this->mode == 1)
		{
			$ws = new ws(array(
							'host' => 'easyfx.nscript.com',
							'port' => '1011',
