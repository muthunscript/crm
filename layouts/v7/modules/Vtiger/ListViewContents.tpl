			...
			<table id="listview-table" class="table {if $LISTVIEW_ENTRIES_COUNT eq '0'}listview-table-norecords {/if} listview-table ">
				...
				{/php}*}
					{foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=listview}
						...
						{$LISTVIEW_ENTRY->getRaw("depositstatus")}
						{php}exit();{/php}*}
					...
					{if $CREDIT eq 1}
                     ...
                     <td class="acceptt"><a href="deposit_credit.php?depositid={$DATA_ID}&type=1&current_url={$ACTUAL_LINK}"><button> Approve </button></a></td>
                    <td class="acceptt declm"><a href="deposit_credit.php?depositid={$DATA_ID}&type=2&current_url={$ACTUAL_LINK}"><button> Decline </button></a></td>
					{/if}
					
					{*depositstatus*}
					{if $MODULE eq "deposit" and $SETTINGS["deposit"] eq 0}
						{if $LISTVIEW_ENTRY->getRaw("status") eq 'Pending'}
							 <td class="acceptt"><a href="deposit_request.php?depositid={$DATA_ID}&type=1&current_url={$ACTUAL_LINK}"><button> Approve </button></a></td>
							 <td class="acceptt declm"><a href="deposit_request.php?depositid={$DATA_ID}&type=2&current_url={$ACTUAL_LINK}"><button> Decline </button></a></td>
						{else}
							<td></td>
							<td></td>
						{/if}
					{elseif $MODULE eq "withdraw" and $SETTINGS["withdraw"] eq 0}
						{if $LISTVIEW_ENTRY->getRaw("status") eq 'Pending'}
							 <td class="acceptt"><a href="withdraw_request.php?withdrawid={$DATA_ID}&type=1&current_url={$ACTUAL_LINK}"><button> Approve </button></a></td>
							<td class="acceptt declm"><a href="withdraw_request.php?withdrawid={$DATA_ID}&type=2&current_url={$ACTUAL_LINK}"><button> Decline </button></a></td>
						{else}
							<td></td>
							<td></td>
						{/if}
