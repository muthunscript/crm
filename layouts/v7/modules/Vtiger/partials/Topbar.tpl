													...
													{if $quickCreateModule == '1'}
														{if $count % 3 == 0}
															...
															{else}
																...
																<div class="{if $hideDiv}create_restricted_{$moduleModel->getName()} hide{else}col-lg-4{/if}">
																	<a id="menubar_quickCreate_{$moduleModel->getName()}" class="quickCreateModule {if $custommodelid gt 52 || $custommodelid eq 27}custom-icon-container{/if} {$custommodelid}" data-name="{$moduleModel->getName()}"
																	   data-url="{$moduleModel->getQuickCreateUrl()}" href="javascript:void(0)">
																		{$moduleModel->getModuleIcon()}
