					...
					<div class="dropdown app-modules-dropdown-container">
						...
						<ul class="dropdown-menu app-modules-dropdown" aria-labelledby="{$APP_NAME}_modules_dropdownMenu">
							{foreach item=moduleModel key=moduleName from=$APP_GROUPED_MENU[$APP_NAME]}
								...
								<li>
									<a href="{if $translatedModuleLabel eq 'Sales'}index.php?module=Reports&view=Detail&record=20{else if $translatedModuleLabel neq 'Finance' && $translatedModuleLabel neq 'Marketing'  && $translatedModuleLabel neq 'Management'  && $translatedModuleLabel neq 'Trading' && $translatedModuleLabel neq 'Service Enquiry'  && $translatedModuleLabel neq 'Product Enquiry' && $translatedModuleLabel neq 'Order Management' && $translatedModuleLabel neq 'Offers' && $translatedModuleLabel neq 'Payment Source'}{$moduleModel->getDefaultUrl()}&app={$APP_NAME}{else}index.php?module=livechat&view=List&app=SUPPORT{/if}" title="{$translatedModuleLabel}">
										<span class="module-icon {if $custommodelid gt 52}custom-icon-container{/if} {$custommodelid}">{$moduleModel->getModuleIcon()}</span>
