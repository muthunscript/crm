...
<div class="">
<input type="hidden" id="lowrisk1" value="{$LOWRISK}" >
<input type="hidden" id="mediumrisk1" value="{$MEDIUMRISK}" >
<input type="hidden" id="highrisk1" value="{$HIGHRISK}">
</div>
...
{*
Array ( [profit] => 34350 [balance] => 0 [margin] => 0 [float] => 68700 [chart_data] => ,34350 [risk_level] => 0 [login] => 1000222 [open_orders] => Array ( [0] => Array ( [ticket] => [symbol] => [volume] => [profit_user] => 34350 ) ) )
*}

{counter assign=inc start=0 print=false}
{foreach key=myId item=i from=$OPENORDER}

{assign var="balance" value=$i["profit"]}
...
<div class="acc_risk">
<div class="account-row" id="accrow{$i["login"]}">
<div class="accounts-box">

{if $balance gte $HIGHRISKMIN and $balance lte $HIGHRISKMAX}
    <div class="risk high_risk"><img src="images/1.png"></div>
{elseif $balance gte $MEDIUMRISKMIN and $balance lte $MEDIUMRISKMAX}
    <div class="risk medium_risk"><img src="images/2.png"></div>
{elseif $balance gte $LOWRISKMIN and $balance lte $LOWRISKMAX}
...
<tr><th colspan="3">ID {$i["login"]}</th></tr>
<tr><td>Bal</td><td> : </td><td>{$i["balance"]}</td></tr>  
<tr><td>Margin</td><td> : </td><td>{$i["margin"]}</td></tr>
<tr><td>Floating</td><td> : </td><td>{$i["profit"]}</td></tr>
</tbody>
</table>
...
<div class="intelligence-box-in">
  <div id="container{$inc}" style="height:198px;"></div>
...
{*{$i["open_orders"][$j]['profit_user']|print_r}

{php}exit();{/php}*}

   <tr>
   <td>#{$i["open_orders"][$j]['ticket']}</td>
   <td>{$i["open_orders"][$j]['symbol']}</td>
   <td>{$i["open_orders"][$j]['volume']}</td>
   <td>{$i["open_orders"][$j]['profit_user']}</td>
   </tr>
...
</tbody></table><div class="view-all" onclick="view_all({$i['login']})">View all</div></div></div>

{*{$i["chart_data"]}
{php}exit();{/php}*}

{*<script>
chart_details({$inc},'{$i["chart_data"]}');
</script>*}
{counter}
