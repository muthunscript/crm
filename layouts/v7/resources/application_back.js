...
/*  
	...
	console.log("_USERMETA........."+_USERMETA.id);

	var page=window.location.href;
	var userid=_USERMETA.id;

	$.ajax({
			url:'onlinestatus_ajax.php',
			type:'post',
...
function getNotifications(){
	...
	$.ajax({
		...
		if(data.success == true){
			$('.notification_container').append(data.htmlContent);
			var totalNotifications = $('.global_notification a').text();
			if(totalNotifications != ''){
				totalNotifications = Number(totalNotifications)+Number(data.totalRows);
			}
			$('.global_notification a').text(totalNotifications);
			if(data.lastNotifyTime && data.lastNotifyTime != ''){
				localStorage.setItem('lastNotifyTime', data.lastNotifyTime);
			}
...
$(document).ready(function(){
	...
	$(document).on('click', function(e){
		if(!($(e.target).hasClass('notification_container') == true || $(e.target).parents('.notification_container').length > 0)){
			if(!($(e.target).hasClass('global_notification') == true || $(e.target).parents('.global_notification').length > 0)){
				$('.notification_container').hide();
			}
		}
	});
	$('.global_notification a').click(function(){
		if($('.notification_container:visible').length > 0){
			$(this).next('.notification_container').hide();
		} else {
			$(this).next('.notification_container').show();
		}
	});

	// Manage Notification Page

	if($('[name="module"]').length > 0 && $('[name="module"]').val() == 'managenotification'){
		$('.editViewHeader').hide();
		$('#multi_user').select2();
		$('#multi_modules').select2();
		$('#managenotification_editView_fieldName_userid').parents('tr').hide();
		$('#managenotification_editView_fieldName_allusers').on('change', function(){
			if($(this).prop('checked') == true){
				$('#multi_user').attr('disabled', true);
			} else {
				$('#multi_user').attr('disabled', false);
			}
		});
		$('#managenotification_editView_fieldName_allmodules').on('change', function(){
			if($(this).prop('checked') == true){
				$('#multi_modules').attr('disabled', true);
			} else {
				$('#multi_modules').attr('disabled', false);
			}
		});
		$('#managenotification').on('submit', function(){
			//console.log($('#multi_user').val());
			//return false;
		});
	}
});
