	...
	//	$returnrecord= $request->get('returnrecord');
		...
		if(!empty($sourceModule)) {
			if(method_exists($moduleModel, 'getQueryByModuleField')) {
				...
				if($moduleName=="mt4account" && $src_field="login_id" && $sourceModule=="withdraw")
				{
					//echo "innnnnnnn";
					//exit();
					...
					if($position) {
						...
						$listQuery = $split[0] . ' LEFT JOIN vtiger_crmentityrel ON vtiger_mt4account.mt4accountid = vtiger_crmentityrel.crmid WHERE ' . $split[1] . ' AND ' . $condition;
					} else {
