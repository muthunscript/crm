...
public function __construct() {
	...
	public function getListViewHeaders() {
		...
		//echo var_dump($headerFieldModels);
		//exit();
	...
	public function getListViewEntries($pagingModel) {
		...
		//echo var_dump($contact_id_related);
		//exit();
		
		
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$current_user_id=$currentUser->get('id');
		$current_user_role=$currentUser->get('roleid');
		//echo var_dump($current_user_id);
		//echo var_dump($current_user_role);
		//exit();
		...
		*/
		...
		/******start*****/
		$returnrecord = $this->get('returnrecord');
		$src_field = $this->get('src_field');
		
		
		//$returnrecord= $this->get('returnrecord');
		//echo var_dump($returnrecord);
		//echo var_dump($src_field);
		//exit();
		
		
		/******end*****/
		
		//echo "ajax_module ".$ajax_module." src_field ".$src_field." ajax_view ".$ajax_view." sourceModule ".$sourceModule;
		...
		if(!empty($sourceModule)) {
			if(method_exists($moduleModel, 'getQueryByModuleField')) {
				...
				if($moduleName=="mt4account" && $src_field="login_id" && ($sourceModule=="withdraw" || $sourceModule=="deposit"))
				{
					//echo "innnnnnnn";
					//exit();
					...
					if($position) {
						...
						$condition=" vtiger_mt4account.loginid!=0 ";
						if($sourceModule=="withdraw")
						{
							$condition.=" AND vtiger_mt4account.acc_type='Live'";
						}
						
						
						$split = preg_split('/where/i', $listQuery);
						$listQuery = $split[0] . ' LEFT JOIN vtiger_crmentityrel ON vtiger_mt4account.mt4accountid = vtiger_crmentityrel.relcrmid WHERE ' . $split[1] . ' AND vtiger_crmentityrel.crmid='.$returnrecord.' AND vtiger_crmentityrel.module="Contacts" AND vtiger_crmentityrel.relmodule="mt4account" AND ' . $condition;
					} else {
						$listQuery = $listQuery. ' WHERE ' . $condition;
					}
		...
		SELECT vtiger_mt4account.loginid, vtiger_mt4account.acc_type, vtiger_mt4account.mt4accountid FROM vtiger_mt4account INNER JOIN vtiger_crmentity ON vtiger_mt4account.mt4accountid = vtiger_crmentity.crmid LEFT JOIN vtiger_crmentityrel ON vtiger_mt4account.mt4accountid = vtiger_crmentityrel.relcrmid WHERE vtiger_crmentity.deleted=0 AND vtiger_mt4account.mt4accountid > 0 AND vtiger_crmentityrel.crmid=315 AND vtiger_crmentityrel.module="Contacts" AND vtiger_crmentityrel.relmodule="mt4account" AND AND vtiger_mt4account.acc_type='Live'
		
		
		SELECT vtiger_mt4account.loginid, vtiger_mt4account.acc_type, vtiger_mt4account.mt4accountid FROM vtiger_mt4account INNER JOIN vtiger_crmentity ON vtiger_mt4account.mt4accountid = vtiger_crmentity.crmid WHERE vtiger_crmentity.deleted=0 AND vtiger_mt4account.mt4accountid > 0
		...
		*/
		...
		if($moduleName=="deposit" && $pagingModel->get('credit')==1)
		{
			$listQuery .= ' and vtiger_deposit.credits_status=2 and vtiger_deposit.credits>0'; 
		}
		
		if($moduleName=="mt4trade" && $pagingModel->get('loginid_details')==1)
		{
			$listQuery .= ' and vtiger_mt4trade.login='.$pagingModel->get('loginid').''; 
		}
		
		//echo $listQuery;
		//exit();
		
		if($moduleName == 'Contacts' && $current_user_role != 'H2')
		{
			
			//$listQuery .= ' and (vtiger_crmentity.smownerid in ('.$current_user->id.') OR vtiger_crmentity.smownerid in (select id from vtiger_users where reports_to_id in ('.$current_user->id.'))) '; 
			
			$affiliates=$this->getAffiliate_users($current_user->id);
			$affiliates=implode(",", $affiliates);
			$listQuery .= ' and (vtiger_crmentity.smownerid in ('.$affiliates.')) '; 
			
		}
		else if($moduleName == 'Users' && $current_user_role != 'H2')
		{
			$affiliates=$this->getAffiliate_users($current_user->id);
			$affiliates=implode(",", $affiliates);
			$listQuery .= ' and (vtiger_users.id in ('.$affiliates.')) '; 
	...
	/** 
	 * Function to get Affiliate users
	**/
	public function getAffiliate_users($aff) {
		...
		$check_data=array();
			...
			if(!empty($check_data))
			{
				...
				{
					$this->getAffiliate_users($data["id"]);
				}
