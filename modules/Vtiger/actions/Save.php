...
class Vtiger_Save_Action extends Vtiger_Action_Controller {
	...
	public function process(Vtiger_Request $request) {
		...
		//echo '<pre>';
		//print_r($request);
		//print_r('module='.$request->get('returnmodule').'&view='.$request->get('returnview').'&viewname='.$request->get('returnviewname').$request->get('appName').'&loginid='.$request->get('login').'&loginid_details=1');
		//echo '</pre>1';die;
		try {
			...
			if ($request->get('returntab_label')){
				...
				if(isset($_SESSION['loginid_details']) && $_SESSION['loginid_details'] == 1) {
					unset($_SESSION['loginid_details']);
					$loadUrl = 'index.php?module='.$request->get('returnmodule').'&view='.$request->get('returnview').'&viewname='.$request->get('returnviewname').$request->get('appName').'&loginid='.$request->get('login').'&loginid_details=1';
				} else {
					$loadUrl = 'index.php?'.$request->getReturnURL();
				}
	...
	public function saveRecord($request) {
		...
		if($request->get('relationOperation')) {
			...
			//echo $request->get('relationOperation');
			//exit();
