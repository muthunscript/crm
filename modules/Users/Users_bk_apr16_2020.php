...
class Users extends CRMEntity {
	...
	//	echo var_dump($this->column_fields["reports_to_id"]);
		
		//exit();
		
		global $current_user,$log;//$adb added by raju for mass mailing
		...
		/******* start *****/
		/*
		//$select_data="select * from vtiger_users where id=?";
		$select_data="select vtiger_users.*,vtiger_user2role.* from vtiger_users LEFT JOIN vtiger_user2role on vtiger_user2role.userid=vtiger_users.reports_to_id where vtiger_users.id=?";
		$select_data=$this->db->pquery($select_data, array($this->id));
		$select_data=$this->db->fetch_array($select_data);
		
		$log->info("select_data ".json_encode($select_data)."");
		
		if($select_data["reports_to_id"]!="")
		{
			$updateQuery = "update vtiger_user2role set roleid=? where userid=?";
			$this->db->pquery($updateQuery,array($select_data["roleid"],$this->id));
		}
		*/
	...
	function insertIntoEntityTable($table_name, $module, $fileid='') {
		...
		if($insertion_mode == 'edit') {
			...
			$hello = $this->db->getLastInsertID();
			
			$log->info("vtiger_users hello ".$this->id."");
			
			if($table_name=="vtiger_users")
			{
				
				$update_params1=array();
				$affiliate_id = ($qparams[60]=='Affiliate')?'AFF00'.$hello:'USER00'.$hello;
				$sql2 = "update $table_name set affiliate_id=? where id=?";
				array_push($update_params1, $affiliate_id);
				array_push($update_params1, $hello);
				$this->db->pquery($sql2, $update_params1);
	...
	function save($module_name, $fileid='') {
		...
		if(isset($this->column_fields['roleid'])) {
			...
			$roleid_update=$this->column_fields['roleid'];
			
			/****start****/
			if(isset($this->column_fields['reports_to_id'])) {
				
				$select_data="select vtiger_users.*,vtiger_user2role.* from vtiger_users LEFT JOIN vtiger_user2role on vtiger_user2role.userid=vtiger_users.reports_to_id where vtiger_users.id=?";
				$select_data=$adb->pquery($select_data, array($this->id));
				$select_data=$adb->fetch_array($select_data);
				
				$log->info("select_data ".json_encode($select_data)."");
		
				if($select_data["reports_to_id"]!="")
				{
					$roleid_update=$select_data["roleid"];
				}
			...
			updateUser2RoleMapping($roleid_update,$this->id);
		}
