...
class Leads_Record_Model extends Vtiger_Record_Model {
	...
	/**
	 * Function returns vtiger_mt4account for Lead Convert
	 * @return Array
	 */
	function getmt4accountFieldsForLeadConvert() {
		$potentialFields = array();
		$privilegeModel = Users_Privileges_Model::getCurrentUserPrivilegesModel();
		$moduleName = 'mt4account';
		...
		if ($moduleModel->isActive()) {
            ...
            $complusoryFields = array('accountnew_id');//array('accountnewid','leverage');
			foreach($fieldModels as $fieldName => $fieldModel) {
	...
	function getConvertLeadFields() {
		...
		*/
		...
		/*
		$Mt4accountFields = $this->getmt4accountFieldsForLeadConvert();
		if(!empty($Mt4accountFields)) {
			$convertFields['mt4account'] = $Mt4accountFields;
		}
