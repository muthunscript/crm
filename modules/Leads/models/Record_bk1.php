...
class Leads_Record_Model extends Vtiger_Record_Model {
	...
	/**
	 * Function returns accountnew fields for Lead Convert
	 * @return Array
	 */
	function getmt4accountFieldsForLeadConvert() {
		...
		if ($moduleModel->isActive()) {
            ...
            $complusoryFields = array('loginid');
            foreach($fieldModels as $fieldName => $fieldModel) {
                if($fieldModel->isMandatory() &&  $fieldName != 'assigned_user_id' && $fieldName != 'account_id') {
                    $keyIndex = array_search($fieldName,$complusoryFields);
                    if($keyIndex !== false) {
	...
	function getConvertLeadFields() {
		...
		if(!empty($accountnewFields)) {
			$convertFields['loginid'] = $mt4accountFields;
		}
		*/
		/********/
