...
class PearDatabase{
	...
	function connect1($dbHostName,$userName,$userPassword,$dbName,$dieOnError = false) {
		
		global $dbconfigoption,$dbconfig,$log;
		...
		$result = $this->database->PConnect($dbHostName, $userName, $userPassword, $dbName);
		if ($result) {
			$this->database->LogSQL($this->enableSQLlog);
