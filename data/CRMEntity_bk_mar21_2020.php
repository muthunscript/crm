...
class CRMEntity {
	...
	static function getInstance($module) {
		...
		//	echo "modules/$module/$modName.php";
		//	exit();
	...
	function saveentity($module, $fileid = '') {
		...
		global $current_user, $adb,$log,$con_id,$withdraw_id,$deposit_id,$mode_c; //$adb added by raju for mass mailing
		...
		//$contactId = $con_id;
			...
			$log->info("insertion_mode ".$insertion_mode);
				//if($mode_c == 1 && $module == "Contacts" )//vtiger_contactdetails && $table_name == "vtiger_contactscf"
				if($con_id!="" && $mode_c==1)
				{
					$mode_c=0;
					$contactId=$value[0];
					...
					$lead_data = $adb->pquery($lead_data, array($con_id));
					$lead_data=$adb->fetch_array($lead_data);
					
					$log->info("lead data ".json_encode($lead_data));
					...
					//$accountnewIdComponents = vtws_getIdComponents($lead_data['accountnewId']);
				...
				$params2 = array($current_id, $login_id, '1', '1', $passsword, $investor_pwd, $lead_data['accountnewid'],'100','Live');
				...
				$params3 = array($con_id, 'Contacts', $current_id, 'mt4account');
			...
			if($withdraw_id!="" && $withdraw_id!=0)
			{
				$log->info("vtiger_withdraw start");
				$log->info("withdraw_id ".$withdraw_id);
				$withdraw_data="select vtiger_crmentity.*,vtiger_withdraw.*,vtiger_mt4account.*   
						...
						where vtiger_crmentity.crmid=?";
				$withdraw_data = $adb->pquery($withdraw_data, array($withdraw_id));
				...
				$withdraw_response =withdraw($login, $withdraw_data["amount"], $mode);
				
				$log->info("withdraw response $withdraw_response");
				
				
				if (strpos($withdraw_response, 'SUCCESSFUL') !== false)
				{
					//UPDATE `vtiger_withdraw` SET `status`=1 WHERE `withdrawid`=
					$adb->pquery('UPDATE vtiger_withdraw SET status=1 WHERE `withdrawid`= ?',array($withdraw_id));
			    }
				
				
				
				
				$log->info("vtiger_withdraw end");
			}
			...
			{
				$log->info("vtiger_deposit start");
				$log->info("deposit_id ".$deposit_id);
				...
				/****************/
				/*
				$deposit_data="select vtiger_crmentity.*,vtiger_deposit.*,vtiger_mt4account.*,vtiger_accountnew.*   
				from vtiger_crmentity 
				left join vtiger_deposit ON vtiger_deposit.depositid= vtiger_crmentity.crmid 
				left join vtiger_mt4account ON vtiger_mt4account.mt4accountid= vtiger_deposit.loginid 
				left join vtiger_accountnew ON vtiger_accountnew.accountnewid= vtiger_mt4account.accountnewid 
				where vtiger_crmentity.crmid=?";
				
				*/
				/****************/
						
				$deposit_data = $adb->pquery($deposit_data, array($deposit_id));
				...
				/*
				$deposit="select * from `vtiger_deposit` where loginid=".$deposit_data['mt4accountid']." and status=1";
				...
				$offers=$adb->pquery("SELECT vtiger_offers.* FROM `vtiger_offers`LEFT JOIN vtiger_crmentity on vtiger_crmentity.crmid=vtiger_offers.offersid WHERE vtiger_crmentity.deleted=0 and visible=1");
				   ...
				   if($offers)
				   {
						...
						while ($row = $adb->fetch_array($offers))
						{
							$ret[]=$row;
						}
				...
				$log->info("offers list ".json_encode($ret));
				$credit_points=0;
				
				foreach($ret as $offer)
				{
					$log->info("each offers ".json_encode($offer));
					$log->info("offers condition".$offer["accountnewid"]."==".$deposit_data["accountnewid"]." ".$offer["deposit_type"]."==".$deposit_type." ".$offer["deposit"]."==".$deposit_data["amount"]);
					if($offer["accountnewid"]==$deposit_data["accountnewid"] && $offer["deposit_type"]==$deposit_type && $offer["deposit"]==$deposit_data["amount"])
					{
				...
				*/
				...
				$credit_array=credit_points($deposit_data['mt4accountid'],$deposit_data["accountnewid"],$deposit_data["amount"]);
				
				$credit_array=json_decode($credit_array,true);
				
				$log->info("credit_points entity".$credit_array["credit_points"]);
				
				$credit_points=$credit_array["credit_points"];
				$offersid=$credit_array["offersid"];
				if($credit_points>0)
				{
					if($json["offers"]==1) //automatic
					{
						//function call
					}
					else //manual
					{
						$adb->pquery('UPDATE vtiger_deposit SET credits_status=2,credits='.$credit_points.',offersid='.$offersid.' WHERE `depositid`= ?',array($deposit_id));
					}
				}
				
				/********credit end*******/
				
				$deposit_response =deposit($login, $deposit_data["amount"], $mode);
				
				$log->info("deposit response ".$deposit_response);
				
				
				if (strpos($deposit_response, 'DEPOSIT SUCCESSFUL') !== false)
				{
					
					$adb->pquery('UPDATE vtiger_deposit SET status=1 WHERE `depositid`= ?',array($deposit_id));
			    }
			
				$log->info("vtiger_deposit end");
			}
	...
	function insertIntoEntityTable($table_name, $module, $fileid = '') {
		...
		//echo var_dump($fileid);
		//exit();
		
		//echo "insertIntoEntityTable........";
		//exit();
		/************/
		
		global $log,$con_id,$withdraw_id,$deposit_id,$mode_c;
		global $current_user, $app_strings;
		$log->info("function insertIntoEntityTable " . $module . ' vtiger_table name ' . $table_name);
		global $adb;
		$insertion_mode = $this->mode;
		//$mode_c=$this->mode;
		//$log->info("mode_c " . $mode_c);
		...
		if ($insertion_mode == 'edit') {
			...
			if(count($changedFields) > 0) {
				...
				/*
				if($table_name=="vtiger_mt4trade")
				{
					$params_data = array();
					$sql_data="select * from vtiger_mt4trade where vtiger_mt4tradeid=?";
					array_push($params_data, $this->id);
					$sql_data=$adb->pquery($sql_data, $params_data);
				...
				*/
				...
				if($table_name=="vtiger_mt4account")
				{
					$params_data = array();
					$sql_data="select * from vtiger_mt4account where mt4accountid=?";
					array_push($params_data, $this->id);
					$sql_data=$adb->pquery($sql_data, $params_data);
					$sql_data=$adb->fetch_array($sql_data);
					...
					foreach($changedFields as $field) {
						...
						if(!is_null($fieldColumn))
						{
							...
							if($fieldColumn=="password")
							{
								
								$response=change_pwd($sql_data["loginid"],$updateFieldValues[$fieldColumn],$sql_data["acc_type"]);	
							...
							else if($fieldColumn=="investor_password")
							{
								$response=change_investor($sql_data["loginid"],$updateFieldValues[$fieldColumn],$sql_data["acc_type"]);	
								$response=trim($response);
							...
							//	leverage
							else if($fieldColumn=="leverage")
							{
								$response=change_leverage($sql_data["loginid"],$updateFieldValues[$fieldColumn],$sql_data["acc_type"]);	
								$response=trim($response);
				...
				/*******start*******/
				if(isset($_REQUEST["loginid_details"])&&$_REQUEST["loginid_details"]==1&&$table_name=="vtiger_mt4trade")
				{
					$_SESSION["loginid_details"]=1;
					
				//	echo "loginid_details.........";
				}
				...
				if(isset($_REQUEST["close_order"])&&$_REQUEST["close_order"]==1&&$table_name=="vtiger_mt4trade")
				{
					//echo "close_order.........";
					
					//order_close($login,$deal,$openprice,$closeprice,$profit,$sl,$tp,$comment,$symbol,$mode)
					
					$order_close=order_close($_REQUEST["login"],$_REQUEST["ticket"],$_REQUEST["open_price"],$_REQUEST["close_price"],$_REQUEST["profit"],$_REQUEST["sl"],$_REQUEST["tp"],$_REQUEST["comment"],$_REQUEST["symbol"],$mode);
					
					$log->info("order_close called..........");
					
					//DEAL Closed Successfully
					
					if($order_close=="DEAL Closed Successfully")
					{
						/*
				...
				if (count($update) > 0) {
					...
					if($table_name=="vtiger_mt4trade")
					{
						...
						//echo "vtiger_mt4trade".$this->id;
						//exit();
						$params_data = array();
						$sql_data="select * from vtiger_mt4trade where mt4tradeid=?";
						array_push($params_data, $this->id);
						$sql_data=$adb->pquery($sql_data, $params_data);
						$sql_data=$adb->fetch_array($sql_data);
						...
						//else
						if(isset($_REQUEST["edit_order"])&&$_REQUEST["edit_order"]==1&&$table_name=="vtiger_mt4trade")
						{
							//$order_edit_delete=order_edit_delete($sql_data["login"],$sql_data["ticket"],$sql_data["open_price"],$sql_data["close_price"],$sql_data["profit"],$sql_data["sl"],$sql_data["tp"],$mode);
							
							
							$order_edit_delete=order_edit_delete($_REQUEST["login"],$_REQUEST["ticket"],$_REQUEST["open_price"],$_REQUEST["close_price"],$_REQUEST["profit"],$_REQUEST["sl"],$_REQUEST["tp"],$mode);
							
							$log->info("order_edit_delete called..........");
						
							//DEAL Edited Successfully
							if($order_edit_delete=="DEAL Edited Successfully")
							{
								$sql1 = "UPDATE $table_name SET " . implode(",", $update) . " WHERE " . $this->tab_name_index[$table_name] . "=?";
								array_push($update_params, $this->id);
								if(Vtiger_Functions::isUserSpecificFieldTable($table_name, $module)){
			...
			$log->info("value id........  " . $value[0]);
			
			$adb->pquery($sql1, $value);
			
			/********Mt4 account start*******/
			if($module == "Contacts" && $table_name == "vtiger_contactscf")
			{
			...
			/********withdraw start*******/
			if($module == "withdraw" && $table_name == "vtiger_crmentity_user_field")
			{
			...
			/********deposit start*******/
			if($module == "deposit" && $table_name == "vtiger_crmentity_user_field")
			{
	...
	function save_related_module($module, $crmid, $with_module, $with_crmid) {
		...
		foreach ($with_crmid as $relcrmid) {
			...
			if ($with_module == 'Documents') {
				...
				if($with_module=="mt4account" && $module=="Contacts")
				{
					$log->info("add account vtiger_mt4account".$crmid);
					$passsword=valid_pwd1(11);
					$ph_passsword=valid_pwd1(5);
					$investor_pwd=valid_pwd1(11);
					
					
					//vtiger_mt4account
					
					$vtiger_mt4account="select vtiger_crmentity.*,vtiger_mt4account.* 
					from vtiger_crmentity 
					left join vtiger_mt4account ON vtiger_mt4account.mt4accountid= vtiger_crmentity.crmid 
					where vtiger_crmentity.crmid=?";
					$vtiger_mt4account = $adb->pquery($vtiger_mt4account, array($relcrmid));
					$vtiger_mt4account=$adb->fetch_array($vtiger_mt4account);
					
					
					
					$lead_data="select vtiger_crmentity.*,vtiger_contactdetails.* 
					from vtiger_crmentity 
					left join vtiger_contactdetails ON vtiger_contactdetails.contactid= vtiger_crmentity.crmid 
					where vtiger_crmentity.crmid=?";
					
					$log->info("select vtiger_crmentity.*,vtiger_contactdetails.* 
					from vtiger_crmentity 
					left join vtiger_contactdetails ON vtiger_contactdetails.contactid= vtiger_crmentity.crmid 
					where vtiger_crmentity.crmid=".$crmid."");
						
					$log->info("contact id ".$crmid);	
						
					$lead_data = $adb->pquery($lead_data, array($crmid));
					$lead_data=$adb->fetch_array($lead_data);
					
					$log->info("contact data ".json_encode($lead_data));
					
					
					//echo var_dump($lead_data);
					//exit();
					
					
					//if($lead_data["isconvertedfromlead"]==1)
					{
						
						$sql = 'SELECT * from vtiger_accountnew where accountnewid=?';
						$account_master = $adb->pquery($sql, array($vtiger_mt4account["accountnewid"]));
						$account_master=$adb->fetch_array($account_master);
						$group = $account_master['title'];
						...
						$comment="Demo Account Online ^".$vtiger_mt4account["accountnewid"];
						
						
						$response=add_account($group,$name,$passsword,$ph_passsword,1,$vtiger_mt4account["leverage"],$comment,$investor_pwd,$email,$vtiger_mt4account["acc_type"]);
						...
						if (strpos($response,'OK') !== false)
						{
							...
							$sql = "UPDATE vtiger_mt4account SET loginid = '".$login_id."',response = 1,pri_mary = 1,password = '".$passsword."',investor_password = '".$investor_pwd."' where mt4accountid=?";
							$result = $adb->pquery($sql, array($with_crmid));
						}
