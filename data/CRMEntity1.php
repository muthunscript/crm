...
class CRMEntity {
	...
	function saveentity($module, $fileid = '') {
		...
		/*	
			...
			if($module=="Leads")
			{
				$lead_data="select vtiger_crmentity.*,vtiger_leaddetails.*,vtiger_leadaddress.* from vtiger_crmentity left join vtiger_leadaddress ON vtiger_leadaddress.leadaddressid= vtiger_crmentity.crmid left join vtiger_leaddetails ON vtiger_leaddetails.leadid= vtiger_crmentity.crmid where vtiger_crmentity.crmid=".$this->id."";
				
				$lead_data = $adb->pquery($lead_data);
				$lead_data=$adb->fetch_array($lead_data);
				
			if($lead_data["mt4_status"]==0 && $lead_data["mt4_status"])
	...
	function insertIntoEntityTable($table_name, $module, $fileid = '') {
		...
		//echo var_dump($key_val);
		//exit();
		/**
		
		{ ["salutation"]=> string(3) "Mr." ["firstname"]=> string(3) "Pri" ["lead_no"]=> string(4) "LEA9" ["lastname"]=> string(1) "p" ["company"]=> string(0) "" ["designation"]=> string(0) "" ["email"]=> string(18) "lavanya@nscript.in" ["leadsource"]=> string(0) "" ["industry"]=> string(0) "" ["leadstatus"]=> string(0) "" ["annualrevenue"]=> int(0) ["rating"]=> string(0) "" ["noofemployees"]=> int(0) ["secondaryemail"]=> string(18) "lavanya@nscript.in" ["emailoptout"]=> string(1) "0" ["tags"]=> string(0) "" ["accountnewid"]=> int(0) }
		
		**/
		
		/*
		$name=$key_val["firstname"]." ".$key_val["lastname"];
				$email=$key_val["email"];
				$agent=$key_val["firstname"]." ".$key_val["lastname"];
				$country="IN";
				$state="TN";
				$city="CH";
				$address="address";
