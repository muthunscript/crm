				...
				*/
				...
				if($credit_points>0)
				{
					if($json["offers"]==1) //automatic
					{
						...
						//credit($login, $credit_points, $expire, $mode)
					}
					else //manual
					{
				...
				if($json["deposit"]==1)//automatic
				{
					...
					if ($deposit_response['status'] == 'OK')
					{
						...
						$adb->pquery('UPDATE vtiger_deposit SET status=1,ticket=? WHERE `depositid`= ?',array($deposit_response['order'],$deposit_id));
					}
					else
					{
						$adb->pquery('UPDATE vtiger_deposit SET status=3 WHERE `depositid`= ?',array($deposit_id));
					}
				}
				else//manual
				{
					$adb->pquery('UPDATE vtiger_deposit SET status=2 WHERE `depositid`= ?',array($deposit_id));
				}
